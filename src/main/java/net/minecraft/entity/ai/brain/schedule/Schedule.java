...
public class Schedule {
   ...
   protected List<ScheduleDuties> getAllDutiesExcept(Activity activityIn) {
      return this.activityToDutiesMap.entrySet().stream().filter((p_221378_1_) -> {
         return p_221378_1_.getKey() != activityIn;
      }).map(Entry::getValue).collect(Collectors.toList());
   }

   public Activity getScheduledActivity(int dayTime) {
      return this.activityToDutiesMap.entrySet().stream().max(Comparator.comparingDouble((p_221376_1_) -> {
         return (double)p_221376_1_.getValue().updateActiveDutyTime(dayTime);
